...
if __name__ == '__main__':
    ...
    keyword = "TUM"
    scene_id = "2_desk_with_person"
    src_cam = keyword+"2.intrinsic"
    ...
    sess.post(FACADE_SERVER_ADDR + "/Load?keyword=" + keyword + "&src=" + src3)
    sess.post(FACADE_SERVER_ADDR + "/Load?keyword=" + keyword + "&src=" + src_cam)
    # 데이터셋의 id 리스트 획득
    res = sess.post(FACADE_SERVER_ADDR + "/Get?keyword=" + keyword + "&src=" + src1)
    ids = np.frombuffer(res.content, dtype=np.uint32)

    res = sess.post(FACADE_SERVER_ADDR + "/Download?keyword=" + keyword + "&id=0"+"&src=" + src_cam, "")
    intrinsic = np.frombuffer(res.content, dtype=np.float32)
    print(intrinsic)
    ...
    for id in ids:
        ...
        res = sess.post(FACADE_SERVER_ADDR + "/Download?keyword=" + keyword + "&id=" + str(id) + "&src=" + src3, "")
        print("ts", float(res.content))
